...
export default function PlayerMessagesPage() {
  ...
  const fetchCoaches = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/auth/athlete/my-coaches/`, {
        credentials: "include",
      });

      if (response.ok) {
        const data = await response.json();
        // Handle both array and object with coaches property
        const coachesList = Array.isArray(data) ? data : (data.coaches || []);
        // Map to expected format with coach user ID
        const mappedCoaches = coachesList.map((c: any) => ({
          id: c.coach, // Use coach user ID, not assignment ID
          username: c.coach_username || c.coach_name,
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-8 py-6 space-y-6">
      <header>
        ...
        <p className="text-sm text-gray-500 mt-1">Communicate with your coaches</p>
      </header>
              ...
              >
                ...
                {coaches.map((coach) => (
                  <option key={coach.id} value={coach.id}>
                    {coach.coach_name} (@{coach.username})
                  </option>
                ))}
              </select>
            </div>
